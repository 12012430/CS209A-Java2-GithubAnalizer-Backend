...
public class RepoController {
  ...
  public List<Developer> developers(@PathVariable("repo_id") int repo_id) {
    ...
    try {
      ...
      java.sql.ResultSet rs = stmt.executeQuery(
          "select Count(developer_name) as num ,repo_id,developer_name  from commit where repo_id= "
              + repo_id + " group by developer_name,repo_id order by num desc limit 9");
      while (rs.next()) {
  ...
  public List<IssueStatus> issuestatus(@PathVariable("repo_id") int repo_id) {
    ...
    try {
      java.sql.Statement stmt = con.createStatement();
      java.sql.ResultSet rs = stmt.executeQuery(
          "select count(issue_id) as num, state from issue where repo_id=" + repo_id
              + " group by state");
      ...
      while (rs.next()) {
        ...
        System.out.println(state);
          if (state.equals("open")) {
              d.setOpen_issues(rs.getInt("num"));
          } else {
              d.setClosed_issues(rs.getInt("num"));
          }
  ...
  public List<Resolution> resolutions(@PathVariable("repo_id") int repo_id) {
    ...
    try {
      java.sql.Statement stmt = con.createStatement();
      java.sql.ResultSet rs = stmt.executeQuery(
          "select  extract('epoch' from (closed_at-create_at)/3600.00/24.00) as days from issue where repo_id="
              + repo_id + " and closed_at is not null ");
  ...
  public List<CommitBR> commitbr(@PathVariable("repo_id") int repo_id) {
    ...
    try {
      java.sql.Statement stmt = con.createStatement();
      java.sql.ResultSet rs = stmt.executeQuery(
          "select * from release where repo_id=" + repo_id + " order by released_at desc limit 9");
      while (rs.next()) {
  ...
  public List<CommitWeek> commitweeks(@PathVariable("repo_id") int repo_id) {
    ...
    try {
      java.sql.Statement stmt = con.createStatement();
      java.sql.ResultSet rs = stmt.executeQuery(
          "select extract(dow from committed_at::date AT TIME ZONE 'Asia/shanghai')+1 as week from commit where repo_id="
              + repo_id);
  ...
  public List<CommitDay> commitdays(@PathVariable("repo_id") int repo_id) {
    ...
    try {
      java.sql.Statement stmt = con.createStatement();
      java.sql.ResultSet rs = stmt.executeQuery(
          "select extract(hour from committed_at AT TIME ZONE 'Asia/shanghai') as hour from commit where repo_id="
              + repo_id);
